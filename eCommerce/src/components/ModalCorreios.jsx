...
export default function ModalCorreios({setModalCorreios}) {

    const [animate, setAnimate] = useState(true)
    const [days, setDays] = useState({label:'1 dia', value: 1})
    ...
    <BaseModal animate={animate} closeModal={closeModal} width={'lg:w-2/5'} top={'lg:top-20'}>
        ...
        <div className='flex flex-col pt-8 pb-5 px-7 w-full'>
            ...
            <div className='flex flex-col gap-2'>
                ...
                <div className='flex justify-between w-full py-1.5 px-4 items-center rounded-lg border border-gray-300'>
                    <h2 className='font-medium text-sm'>Mini envios</h2>
                    <Switch />
                </div>
            </div>
            ...
            <LoadingButton loading={loading} text='Salvar alterações' full={true} handleSubmit={() => console.log('Foi')} />
        </div>
    </BaseModal>
  )
}

...
export default function ModalProduto({setModalProduto, edit, categorias, idProduto}) {
    ...
    async function handleSubmit() {
        ...
        if(edit){
            updateProduct(idProduto, name, price, priceoff, category, desc, image)
            .then( (data) => {
                setName(data.product.title) 
                setPrice(data.product.preco)
                setDesc(data.product.desc) 
        ...
        addProduct(name, price, priceoff, category, desc, image)
  ...
  return (
    ...
    >
        ...
        <div className='flex items-center justify-between bg-gray-100 w-full px-6 py-4 rounded-b-2xl border-t'>
            ...
            >
                <input className='hidden' type='file' onChange={(ev) => setImage(ev.target.files[0])} />
